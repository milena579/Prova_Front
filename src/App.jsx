...
function App() {
  ...
  const getUsers = async () => {
    try {
      const res = await axios.get("https://prova-back.onrender.com/api/person");
      setList(res.data);
  ...
  const registerUser = async () => {
    try { 
      await axios.post("https://prova-back.onrender.com/api/person",{
  ...
  const deleteUser = async (id) => {
    try { 
      await axios.delete(`https://prova-back.onrender.com/api/person/${id}`);
      getUsers()
    } catch (error) {
      console.error("Erro ao inserir", error);
    }
